...
def list_transactions():
    ...
    if kind_filter == 'income':
        query = query.filter_by(transaction_kind='income')
    elif kind_filter == 'expense':
        query = query.filter_by(transaction_kind='expense')

    transactions = query.all()

    total_income = db.session.query(
        func.sum(Transaction.amount)
    ).filter_by(transaction_kind='income').scalar() or 0.0

    total_expense = db.session.query(
        func.sum(Transaction.amount)
    ).filter_by(transaction_kind='expense').scalar() or 0.0
    ...
    if form.validate_on_submit():
        ...
        if kind == 'income':
            ...
            transaction = Transaction(
                ...
                paid_amount         = form.expense_amount.data,
                total_amount        = form.expense_amount.data,
                date                = datetime.combine(
                                            form.date.data,
                                            time(0, 0),
                                            tzinfo=timezone.utc,
